...
function CustomTooltip({ active, payload }) {
  ...
  const { payment_method, total_final_amount, total_transactions } =
    payload[0].payload;
  return (
    <div className="rounded-lg border border-border bg-card p-3 text-sm shadow-lg">
      ...
      <p className="text-muted-foreground">Transaksi: {total_transactions}</p>
    </div>
  );
}
            ...
            >
              {chartData.map((entry) => (
                <Cell
                  ...
                  fill={
                    PAYMENT_COLORS[entry.payment_method?.toLowerCase()] ??
